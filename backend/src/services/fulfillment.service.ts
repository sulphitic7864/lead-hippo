...
export async function fulfillPurchase(purchaseId: number, force = false) {
  const rows = await query<PurchaseRow[]>(
    `SELECT p.*,l.lead_code,l.title,l.city,l.spots_remaining FROM purchases p JOIN leads l ON l.id=p.lead_id WHERE p.id=? LIMIT 1`,
    [purchaseId],
  );
  const p = rows[0];
  ...
  if (!reportPath || force) {
    try {
      reportPath = await generateOpportunityReport(purchaseId);
      await pool.execute(
        "UPDATE purchases SET report_path=?,report_status='GENERATED' WHERE id=?",
        [reportPath, purchaseId],
      );
    } catch (error) {
      await pool.execute(
        "UPDATE purchases SET report_status='FAILED' WHERE id=?",
        [purchaseId],
      );
      throw error;
    }
  ...
  try {
    await sendBuyerReport({
      ...
      title: p.title,
      city: p.city,
      leadCode: p.lead_code,
      reportPath,
    });
    await pool.execute("UPDATE purchases SET email_status='SENT' WHERE id=?", [
      purchaseId,
    ]);
  } catch (error) {
    await pool.execute(
      "UPDATE purchases SET email_status='FAILED' WHERE id=?",
      [purchaseId],
    );
    throw error;
  }
