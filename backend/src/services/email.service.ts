...
function getTransporter() {
  ...
  if (!env.SMTP_HOST || !env.SMTP_USER || !env.SMTP_PASSWORD || env.SMTP_PASSWORD === 'replace_me') {
...
export async function sendBuyerReport(input:{to:string;title:string;city:string;leadCode:string;reportPath:string;}) {
  return getTransporter().sendMail({
    from: env.SMTP_FROM,
    to: input.to,
    subject: `Your Lead Hippo Opportunity Report — ${input.title}, ${input.city}`,
    html: shell('Your Opportunity Report is ready', `<p>Thank you for purchasing <strong>${input.title}</strong> in ${input.city}.</p><p>Your branded Opportunity Report is attached. This opportunity is shared with no more than three contractors.</p><p>Please contact the homeowner professionally and use their information only for this project.</p><p>Need help? Email <a href="mailto:${env.SUPPORT_EMAIL}">${env.SUPPORT_EMAIL}</a>.</p>`),
    attachments: [{ filename:`${input.leadCode}-Opportunity-Report.pdf`, path:input.reportPath }]
  });
}

export async function sendAdminSale(input:{leadCode:string;title:string;company:string;email:string;phone:string;amountCents:number;spotsRemaining:number;}) {
  return getTransporter().sendMail({
    from: env.SMTP_FROM,
    to: env.ADMIN_EMAIL,
    subject: `Lead Hippo sale — ${input.leadCode}`,
    html: shell('New opportunity sale', `<p><strong>${input.title}</strong> (${input.leadCode}) was purchased.</p><table cellpadding="6"><tr><td>Company</td><td><strong>${input.company}</strong></td></tr><tr><td>Email</td><td>${input.email}</td></tr><tr><td>Phone</td><td>${input.phone}</td></tr><tr><td>Amount</td><td>$${(input.amountCents/100).toFixed(2)} CAD</td></tr><tr><td>Spots remaining</td><td>${input.spotsRemaining}</td></tr></table>`)
  });
}

export async function sendContactNotification(input:{name:string;company:string|null;email:string;phone:string;message:string;}) {
  return getTransporter().sendMail({
    from: env.SMTP_FROM,
    replyTo: input.email,
    to: env.ADMIN_EMAIL,
    subject: `Lead Hippo contact form — ${input.name}`,
    html: shell('New contact-form submission', `<p><strong>${input.name}</strong>${input.company?` from ${input.company}`:''}</p><p>${input.email}<br>${input.phone}</p><p style="white-space:pre-wrap">${escapeHtml(input.message)}</p>`)
  });
}
